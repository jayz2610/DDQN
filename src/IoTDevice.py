...
class DeviceList:
    ...
    def get_collected_data(self):
        total_collected = 0
        print(f"[DeviceList GetCollectedData] Devices count: {len(self.devices)}")
        for i, device in enumerate(self.devices):
            print(
                f"  Dev {i}@{device.position} (Color: {device.params.color if hasattr(device.params, 'color') else 'N/A'}): device.collected_data = {device.collected_data:.4f}, device.data = {device.data:.4f}")
            total_collected += device.collected_data
        print(f"[DeviceList GetCollectedData] Sum calculated: {total_collected:.4f}")
